...
export default function AuthPage() {
  ...
  const [signupRole, setSignupRole] = useState<PractitionerRole>("Patient");
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        <div className="text-center space-y-3 mb-8">
          <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/20 text-xs font-semibold">
            ...
            <span>{t("nav.brandSubtitle", "Identity & Access Gateway")}</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
            {t("auth.signIn", "Patient & Clinician")}{" "}
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm max-w-xl mx-auto">
            {t("auth.autoGeneratedNotice", "Create an account to automatically receive your unique Patient Identifier and Record Number.")}
          </p>
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-emerald-300 dark:border-emerald-500/40 rounded-3xl p-8 shadow-2xl space-y-6 max-w-xl mx-auto text-center animate-fade-in">
            ...
            <div className="space-y-1">
              <span className="text-xs font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
                {t("common.verified", "Registration Successful")}
              </span>
              <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">
                Welcome, {newlyRegistered.name}!
              </h2>
              <p className="text-xs text-slate-600 dark:text-slate-300">
                Your medical identity has been registered. Your unique Patient ID and Record Number have been generated automatically:
              </p>
            </div>
            ...
            >
              <span>{t("landing.getStarted", "Enter Patient Assessment Workspace")}</span>
            ...
            <div className="space-y-2.5 font-mono text-xs text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800/80">
              {user.patient_id && (
                <div className="flex justify-between">
                  <span className="text-slate-500 font-sans font-semibold">Auto-Generated Patient ID:</span>
          ...
          <div className="bg-white dark:bg-slate-900/60 backdrop-blur-sm border border-emerald-100 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl shadow-emerald-900/5 dark:shadow-none space-y-6 max-w-xl mx-auto animate-fade-in">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-2xl border border-slate-200 dark:border-slate-800">
              ...
              >
                ...
                <span>{t("auth.signUp", "Patient / User Sign Up")}</span>
              </button>
            </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1.5">
                    ...
                    <span>{t("auth.phone", "Phone Number or Email")}</span>
                  </label>
                  <input
                    type="text"
                    required
                    placeholder="e.g. 9345693386 or user@trustmed.org"
                    value={loginPhone}
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{t("common.loading", "Verifying Credentials...")}</span>
                    </>
                  ) : (
                    <>
                      <LogIn className="w-4 h-4" />
                      <span>{t("auth.loginBtn", "Sign In & Access Records")}</span>
                    </>
                  )}
                </button>
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    disabled={isSubmitting}
                    className="w-full py-2.5 px-4 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/40 font-semibold text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
                  >
                    <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                    <span>1-Click Patient Demo Access (Instant Login)</span>
                  </button>
                </div>
              </form>
            )}
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                      {t("auth.firstName", "First Name")} <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                      {t("auth.lastName", "Last Name")} <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1">
                      <Phone className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                      <span>{t("auth.phone", "Phone Number")} <span className="text-rose-500">*</span></span>
                    </label>
                    <input
                      type="tel"
                      required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1">
                    <Key className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                    <span>{t("auth.password", "Password")} <span className="text-rose-500">*</span></span>
                  </label>
                  <input
                    type="password"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-1">
                    ...
                    <span>{t("auth.address", "Address")}</span>
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. 120 Beacon St, Boston, MA"
                    value={signupAddress}
                ...
                >
                  {isSubmitting ? (
                    <>
                      <RefreshCw className="w-4 h-4 animate-spin" />
                      <span>{t("common.loading", "Registering Account...")}</span>
                    </>
                  ) : (
                    <>
                      <UserPlus className="w-4 h-4" />
                      <span>{t("auth.registerBtn", "Create Account & Auto-Generate Patient ID")}</span>
                    </>
                  )}
                </button>
              </form>
